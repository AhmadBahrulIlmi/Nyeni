...
const UbahKonten = ({route}) => {
      ...
      .onSnapshot(documentSnapshot => {
        ...
        if (dataKonten) {
          ...
          setdataKonten({
            ...
            category: {
              id: dataKonten.category.id,
              name: dataKonten.category.name,
            },
          });
          setOldImage(dataKonten.image);
          setImage(dataKonten.image);
          setLoading(false);
        } else {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await firestore().collection('konten').doc(kontenId).update({
        title: dataKonten.title,
        description: dataKonten.description,
        origin: dataKonten.origin,
        conclusion: dataKonten.conclusion,
      ...
      setLoading(false);
      console.log('Konten Diubah!');
      navigation.navigate('DetailKonten', {kontenId});
    } catch (error) {
      console.log(error);
    }
  };
        ...
        {image ? (
          ...
          <TouchableOpacity onPress={handleImagePick}>
            <View
              ...
              <GalleryAdd color={colors.grey(0.6)} variant="Linear" size={42} />
              <Text
                style={{
                  fontFamily: fontType['Pjs-Regular'],
                  fontSize: 12,
